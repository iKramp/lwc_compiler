...
int genNot(Node& base, std::string& asm_str){
    ...
    asm_str += "NOT  " + std::to_string(writeRealReg(n)) + "   NULL  " + std::to_string(writeRealReg(free_reg)) + "    //binary negate\n";//add result
    return free_reg;
}
...
std::string genFn(Node& base){//already has all vars pushed
    ...
    if(!main) {
        asm_str += "POP  NULL NULL  0    //get ret addr\n";
        asm_str += "JMP  0    NULL  NULL //jump to ret addr\n";
    } else {
        asm_str += "NOP  NULL NULL NULL  //end of program\n";
    }
    return asm_str;
}
...
std::string generate(Node& root){
    std::string asm_str;
    //jump to function 0
    asm_str += "IMM  0    REF_FN_0 //set jump addr\n";
    asm_str += "JMP  0    1    NULL //jump to function 0\n";
